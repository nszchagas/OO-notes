...
class PainelNovo extends JPanel implements ActionListener, ItemListener {
	...
	private boolean isPreenchido() {
		...
		if (txtFieldNome.getText() == null || txtFieldNome.getText().length() == 0
				...
				|| comboBoxSituacaoDeSaude.getSelectedItem() == null) {
			return false;

		} else if (masculino.isSelected()) {
			if (txtFieldIdade.getText() == null || txtFieldIdade.getText().length() == 0)
				return false;
	...
	private void cadastraPessoa() {
		...
		if (feminino.isSelected()) {
			...
			if (!Validacao.isRepetida(new Mulher(txtFieldNome.getText(), situacaoDeSaudePessoa, gravidezPessoa))) {
				new PessoaDAO()
						.cadastraMulher(new Mulher(txtFieldNome.getText(), situacaoDeSaudePessoa, gravidezPessoa));
			...
			if (!Validacao.isRepetida(new Homem(txtFieldNome.getText(), situacaoDeSaudePessoa,
				...
				new PessoaDAO().cadastraHomem(new Homem(txtFieldNome.getText(), situacaoDeSaudePessoa,
						Integer.parseInt(txtFieldIdade.getText())));
				JOptionPane.showMessageDialog(this, "Pessoa cadastrada com sucesso!", "Cadastro",
						JOptionPane.INFORMATION_MESSAGE);
				limpaCampos();
